...
export function buildModel(data, sequence) {
  ...
  const envelopes=data.categories.filter(c=>calculations.some(r=>r.category_id===c.id)).sort((a,b)=>a.display_order-b.display_order).map(c=>{
    ...
    return {id:c.id,category:c.canonical_name,budget_cents:b.amount_cents,actual_cents:r.actual_cents,ending_cents:r.ending_envelope_cents,starting_cents:r.starting_cents,moved_cents:r.moved_cents,source:{sheet:c.source_sheet,row:c.source_row,actual_cell:s.actual_source_cell,ending_cell:s.ending_source_cell}};
  });
