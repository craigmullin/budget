...
parser.add_argument('--seed', default='.local/firebase/seed.json')
parser.add_argument('--output')
args = parser.parse_args()
seed = json.loads(Path(args.seed).read_text())
with XlsxReader(args.workbook) as workbook:
    ...
    for row in range(2, 65):
        ...
        defaults.append({'category_id': category['id'], 'amount_cents': cents(rows[row]['B'].value),
                         'source_cell': f'B{row}'})
    assert sum(d['amount_cents'] for d in defaults) == cents(rows[67]['B'].value)
    assert {d['category_id'] for d in defaults} == {b['category_id'] for b in seed['budget_allocations']}
funded = {a['allocation_period_id'] for a in seed['budget_allocations'] if a['amount_cents'] != 0}
periods = [p for p in seed['allocation_periods'] if p['id'] not in funded and p['label_date'] >= '2026-09-11']
assert periods and all(p['id'] not in funded for p in periods)
assert all(a['amount_cents'] == 0 for a in seed['budget_allocations'] if a['allocation_period_id'] in {p['id'] for p in periods})
config = {'start_date': periods[0]['label_date'], 'opening_cents': 0, 'envelope_ids': [d['category_id'] for d in defaults],
